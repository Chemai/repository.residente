...
def index():
	...
	addDir ('Compilaciones'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)

#	addDir ('[B]Otros Wizards[/B]',       '', 'thirdparty', icon=ICONMAINT, themeit=THEME1)
	
	
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	
	addDir ('Mantenimiento'   ,'maint',    icon=ICONMAINT,    themeit=THEME1)
	if wiz.platform() == 'android' or DEVELOPER == 'true': addDir ('Instalador de Apk' ,'apk', icon=ICONAPK, themeit=THEME1)
	if not ADDONFILE == 'http://': addDir ('Instalador de addons' ,'addons', icon=ICONADDONS, themeit=THEME1)
#	if wiz.platform() == 'android' or wiz.platform() == 'windows' or DEVELOPER == 'true': addDir ('Zona de Juegos Retro'       ,'retromenu', icon=ICONRETRO,     themeit=THEME1)	
	addDir ('Datos Usuario'     ,'savedata', icon=ICONSAVE,     themeit=THEME1)
	...
	if DEVELOPER == 'true': addDir('Menu de desarrollo','developer', icon=ICONSETTINGS, themeit=THEME1)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		...
		addDir ('Datos a guardar'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addFile('Url del archivo no valida', '', icon=ICONBUILDS, themeit=THEME3)
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count15 > 0:
					...
					addFile('[B]%s Isengard y anteriores(%s)[/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME3)
					if SHOW15 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
			...
			if adultcount > 0:
				addFile('Solo hay Compilaciones para adultos', '', icon=ICONBUILDS, themeit=THEME3)
				addFile('Mostarr addons para adultos en Ajustes > Misc', '', icon=ICONBUILDS, themeit=THEME3)
			else:
				addFile('Actualmente no hay compilaciones de %s' % ADDONTITLE, '', icon=ICONBUILDS, themeit=THEME3)
		else: addFile('El archivo de Compilaciones no es correcto.', '', icon=ICONBUILDS, themeit=THEME3)
	setView('files', 'viewType')

def viewBuild(name):
	WORKINGURL = wiz.workingURL(BUILDFILE)
	if not WORKINGURL == True:
		addFile('Url del archivo no valida', '', themeit=THEME3)
		addFile('%s' % WORKINGURL, '', themeit=THEME3)
		return
	if wiz.checkBuild(name, 'version') == False: 
		addFile('Error leyendo archivo.', '', themeit=THEME3)
		addFile('%s no se encontro en listado.' % name, '', themeit=THEME3)
		return
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addDir ('Datos a Guardar',       'savedata', icon=ICONSAVE,     themeit=THEME3)
		addFile('Info de Compilacion',    'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
		if not preview == "http://": addFile('Previsualiza Video', 'buildpreview', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
		...
		if warning == True:
			addFile('[I]Compilacion para la version %s [B][COLOR red](instalada: %s)[/COLOR][/B][/I]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME3)
...
def editThirdParty(number):
	...
	url2  = wiz.getKeyboard(url, 'URL del archivo del Wizard')
...
def youtubeMenu(url=None):
	if not YOUTUBEFILE == 'http://':
		...
		if YOUTUBEWORKING == True:
			...
			else: wiz.log("[YouTube Menu] ERROR:  Formato Invalido.")
		else: 
			wiz.log("[YouTube Menu] ERROR: URL de la lista no es valida.")
			addFile('Url del archivo no valida', '', themeit=THEME3)
			addFile('%s' % YOUTUBEWORKING, '', themeit=THEME3)
	else: wiz.log("[YouTube Menu] No hay lista de videos.")
	setView('files', 'viewType')
...
def maintMenu(view=None):
	...
	errorsfound = str(errorsinlog) + ' Error(s) Found' if errorsinlog > 0 else 'Ninguno'
	...
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('[B]Limpieza Total:[/B] [COLOR green][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize)  ,'fullclean',       icon=ICONMAINT, themeit=THEME3)
	...
	if view == "addon" or SHOWMAINT == 'true': 
		...
		addDir ('Borrar Datos de Addon_Data',              'removeaddondata', icon=ICONMAINT, themeit=THEME3)
		addDir ('Habilita/Desactiva Addons',          'enableaddons',    icon=ICONMAINT, themeit=THEME3)
		...
		addFile('Habilita/Desactiva Addons de adulto',    'toggleadult',     icon=ICONMAINT, themeit=THEME3)

		addFile('Oculta Passwords al teclear',   'hidepassword',   icon=ICONMAINT, themeit=THEME3)
		addFile('Muestra Passwords al teclear', 'unhidepassword', icon=ICONMAINT, themeit=THEME3)
	...
	addFile('[B]-----[/B] Mostrar todo el MENU: %s' % maint.replace('true',on).replace('false',off) ,'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME3)

	addDir ('[I][B]<< << << [/B] Volver al Menu Principal[/I]', icon=ICONMAINT, themeit=THEME3)

		
		
	addDir ('BackUp / Restore'     ,'maint', 'backup',   icon=ICONMAINT, themeit=THEME1)
	if view == "backup" or SHOWMAINT == 'true':
		addFile('[COLOR red][B]Borra archivo de BackUp[/B][/COLOR]',        'clearbackup',     icon=ICONMAINT,   themeit=THEME3)
		addFile('Carpeta del BackUp: [COLOR %s]%s[/COLOR]' % (COLOR2, MYBUILDS),'settings', 'Maintenance', icon=ICONMAINT, themeit=THEME3)
		addFile('* BackUp Config. actual',               'backupbuild',     icon=ICONMAINT,   themeit=THEME3)
		addFile('* BackUp Addon_data',          'backupaddon',     icon=ICONMAINT,   themeit=THEME3)
		addFile('Restaurar BackUp',         'restorezip',      icon=ICONMAINT,   themeit=THEME3)
		addFile('Restaurar Addon_data',    'restoreaddon',    icon=ICONMAINT,   themeit=THEME3)

		addFile('Restaurar BackUp (Ext)',      'restoreextzip',   icon=ICONMAINT,   themeit=THEME3)
		addFile('Restaurar Addon_data (Ext)', 'restoreextaddon', icon=ICONMAINT,   themeit=THEME3)		
...
#		addFile('[Restore]: External GuiFix',     'restoreextgui',   icon=ICONMAINT,   themeit=THEME3)
	...
	if view == "Auto Clean" or SHOWMAINT == 'true':
		...
		addFile('[COLOR lime][B]- Limpieza Automatica al inicio: %s[/B][/COLOR]' % autoclean.replace('true',on).replace('false',off) ,'togglesetting', 'autoclean',   icon=ICONMAINT, themeit=THEME3)
	if autoclean == 'true':
...
#		addFile('--- Borra Cache de Video', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
...
def advancedWindow(url=None):
	if not ADVANCEDFILE == 'http://':
		...
		if os.path.exists(ADVANCED): 
			addFile('Ver archivo actual AdvancedSettings.xml', 'currentsettings', icon=ICONMAINT, themeit=THEME3)
			addFile('[COLOR red][B]Borrar el actual AdvancedSettings.xml[/B][/COLOR]', 'removeadvanced',  icon=ICONMAINT, themeit=THEME3)
...
def saveMenu():
	...
	addDir ('Guardar Datos Trakt',               'trakt',                icon=ICONTRAKT, themeit=THEME1)
	addDir ('Guardar Datos Real Debrid',              'realdebrid',           icon=ICONREAL,  themeit=THEME1)
	addDir ('Guardar Informacion de Login',               'login',                icon=ICONLOGIN, themeit=THEME1)
	addFile('Importar Datos Guardados',              'managedata', 'import', icon=ICONSAVE,  themeit=THEME1)
	addFile('Exportar Datos Guardados',              'managedata', 'export', icon=ICONSAVE,  themeit=THEME1)
	addFile('- Click para cambiar ajustes -', '', themeit=THEME3)
	addFile('Guardar Trakt: %s' % trakt.replace('true',on).replace('false',off)                       ,'togglesetting', 'keeptrakt',      icon=ICONTRAKT, themeit=THEME1)
	addFile('Guardar Real Debrid: %s' % real.replace('true',on).replace('false',off)                  ,'togglesetting', 'keepdebrid',     icon=ICONREAL,  themeit=THEME1)
	addFile('Guardar Login Info: %s' % login.replace('true',on).replace('false',off)                  ,'togglesetting', 'keeplogin',      icon=ICONLOGIN, themeit=THEME1)
	addFile('Guardar \'Sources.xml\': %s' % sources.replace('true',on).replace('false',off)           ,'togglesetting', 'keepsources',    icon=ICONSAVE,  themeit=THEME1)
	addFile('Guardar \'Profiles.xml\': %s' % profiles.replace('true',on).replace('false',off)         ,'togglesetting', 'keepprofiles',   icon=ICONSAVE,  themeit=THEME1)
	addFile('Guardar \'Advancedsettings.xml\': %s' % advanced.replace('true',on).replace('false',off) ,'togglesetting', 'keepadvanced',   icon=ICONSAVE,  themeit=THEME1)
	addFile('Guardar \'Favourites.xml\': %s' % favourites.replace('true',on).replace('false',off)     ,'togglesetting', 'keepfavourites', icon=ICONSAVE,  themeit=THEME1)
	addFile('Guardar Super Favourites: %s' % super.replace('true',on).replace('false',off)            ,'togglesetting', 'keepsuper',      icon=ICONSAVE,  themeit=THEME1)
	addFile('Guardar REPOS Instalados: %s' % repos.replace('true',on).replace('false',off)           ,'togglesetting', 'keeprepos',      icon=ICONSAVE,  themeit=THEME1)
	addFile('Guardar mi \'WhiteList\': %s' % whitelist.replace('true',on).replace('false',off)        ,'togglesetting', 'keepwhitelist',  icon=ICONSAVE,  themeit=THEME1)
	if whitelist == 'true':
		addFile('Editar Whitelist',        'whitelist', 'edit',   icon=ICONSAVE,  themeit=THEME1)
		addFile('Ver Whitelist',        'whitelist', 'view',   icon=ICONSAVE,  themeit=THEME1)
		addFile('Borrar Whitelist',       'whitelist', 'clear',  icon=ICONSAVE,  themeit=THEME1)
		addFile('Importar Whitelist',      'whitelist', 'import', icon=ICONSAVE,  themeit=THEME1)
		addFile('Exportar Whitelist',      'whitelist', 'export', icon=ICONSAVE,  themeit=THEME1)
	setView('files', 'viewType')

def traktMenu():
	trakt = '[COLOR green]ON[/COLOR]' if KEEPTRAKT == 'true' else '[COLOR red]OFF[/COLOR]'
	last = str(TRAKTSAVE) if not TRAKTSAVE == '' else 'Trakt aun no se ha guardado.'
	addFile('[I]Registre cuenta GRATIS en http://trakt.tv[/I]', '', icon=ICONTRAKT, themeit=THEME3)
	addFile('Guarde sus Datos Trakt: %s' % trakt, 'togglesetting', 'keeptrakt', icon=ICONTRAKT, themeit=THEME3)
	if KEEPTRAKT == 'true': addFile('Ultima vez: %s' % str(last), '', icon=ICONTRAKT, themeit=THEME3)
	...
	for trakt in traktit.ORDER:
		...
		if not os.path.exists(path): addFile('[COLOR red]Addon Data: No Instalado[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR red]Addon Data: No Registrado[/COLOR]','authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR green]Addon Data: %s[/COLOR]' % auser,'authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR red]Saved Data: Guardar archivo (Importar datos)[/COLOR]','importtrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR red]Saved Data: No guardar[/COLOR]','savetrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR green]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
	
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Guardar Datos Trakt',          'savetrakt',    'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Recuperar todos los Datos Trakt guardados', 'restoretrakt', 'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Importar Datos Trakt',            'importtrakt',  'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Borrar todos los Datos Trakt guardados',   'cleartrakt',   'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Borrar el Addon Data (Trakt)',         'addontrakt',   'all', icon=ICONTRAKT,  themeit=THEME3)
	setView('files', 'viewType')

def realMenu():
	real = '[COLOR green]ON[/COLOR]' if KEEPREAL == 'true' else '[COLOR red]OFF[/COLOR]'
	last = str(REALSAVE) if not REALSAVE == '' else 'Real Debrid aun no se ha guardado.'
	addFile('[I]http://real-debrid.com es un servicio de PAGO.[/I]', '', icon=ICONREAL, themeit=THEME3)
	addFile('Guardar Datos Real Debrid: %s' % real, 'togglesetting', 'keepdebrid', icon=ICONREAL, themeit=THEME3)
	if KEEPREAL == 'true': addFile('Ultima vez: %s' % str(last), '', icon=ICONREAL, themeit=THEME3)
	...
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Guardar TODOS los Datos Real Debrid',          'savedebrid',    'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Recuperar todos los Datos Real Debrid guardados', 'restoredebrid', 'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Importar Datos Real Debrid',            'importdebrid',  'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Borrar todos los Datos Real Debrid guardados',   'cleardebrid',   'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Borrar el Addon Data (Real Debrid)',               'addondebrid',   'all', icon=ICONREAL,  themeit=THEME3)
	setView('files', 'viewType')

def loginMenu():
	login = '[COLOR green]ON[/COLOR]' if KEEPLOGIN == 'true' else '[COLOR red]OFF[/COLOR]'
	last = str(LOGINSAVE) if not LOGINSAVE == '' else 'Datos de Login aun no se han guardado.'
	addFile('[I]Varios de estos addons son servicios de PAGO.[/I]', '', icon=ICONLOGIN, themeit=THEME3)
	addFile('Save Login Data: %s' % login, 'togglesetting', 'keeplogin', icon=ICONLOGIN, themeit=THEME3)
	if KEEPLOGIN == 'true': addFile('Ultima vez: %s' % str(last), '', icon=ICONLOGIN, themeit=THEME3)
	...
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Guardar todos los datos de inicio de sesion',          'savelogin',    'all', icon=ICONLOGIN,  themeit=THEME3)
	addFile('Recuperar todos los Datos de Login guardados', 'restorelogin', 'all', icon=ICONLOGIN,  themeit=THEME3)
	addFile('Importar Datos de Login',            'importlogin',  'all', icon=ICONLOGIN,  themeit=THEME3)
	addFile('Borrar todos los Datos de  Login guardados',   'clearlogin',   'all', icon=ICONLOGIN,  themeit=THEME3)
	addFile('Borrar los Addon Data',         'addonlogin',   'all', icon=ICONLOGIN,  themeit=THEME3)
	setView('files', 'viewType')
...
def fixUpdate():
	if KODIV < 17: 
		...
		try:
			...
			wiz.log("No se puede eliminar %s, Purging DB" % dbfile)
			wiz.purgeDb(dbfile)
	else:
		xbmc.log("Se elimino Addons.db porque no funciona en Kod17")
...
def removeAddonMenu():
	...
	for folder in sorted(fold, key = lambda x: x):
		...
		if os.path.exists(xml):
			...
			try: 
				...
				pass
	if len(addonnames) == 0:
		wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Ningun Addon para quitar[/COLOR]" % COLOR2)
		return
	if KODIV > 16:
		selected = DIALOG.multiselect("%s: Selecciona los addons a eliminar." % ADDONTITLE, addonnames)
	else:
		selected = []; choice = 0
		tempaddonnames = ["-- Click aqui para Seguir --"] + addonnames
		while not choice == -1:
			choice = DIALOG.select("%s: Selecciona los addons que quieres eliminar." % ADDONTITLE, tempaddonnames)
			if choice == -1: break
			elif choice == 0: break
			else: 
				choice2 = (choice-1)
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar salida[/COLOR] de kodi o [COLOR %s]Recargar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Recargar Perfil[/COLOR][/B]", nolabel="[B][COLOR red]Forzar salida[/COLOR][/B]")
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		addFile('[COLOR red][B][QUITAR][/B][/COLOR] TODO el Addon_Data', 'removedata', 'all', themeit=THEME2)
		addFile('[COLOR red][B][QUITAR][/B][/COLOR] Addon_Data para los Addons desinstalados', 'removedata', 'uninstalled', themeit=THEME2)
		addFile('[COLOR red][B][QUITAR][/B][/COLOR] las carpetas vacias de Addon_Data', 'removedata', 'empty', themeit=THEME2)
		addFile('[COLOR red][B][QUITAR][/B][/COLOR] %s Addon_Data' % ADDONTITLE, 'resetaddon', themeit=THEME2)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			else: folderdisplay = '[COLOR red][B][QUITAR][/B][/COLOR] %s' % folderdisplay
...
def enableAddons():
	addFile("[I][B][COLOR red]!! Aviso: Deshabilitar algunos addons puede causar problemas !![/COLOR][/B][/I]", '', icon=ICONMAINT)
	fold = glob.glob(os.path.join(ADDONS, '*/'))
	x = 0
	for folder in sorted(fold, key = lambda x: x):
		foldername = os.path.split(folder[:-1])[1]
		...
		if os.path.exists(addonxml):
			...
			f.close()
	if x == 0:
		addFile("No se encontraron addons para habilitar o desactivar.", '', icon=ICONMAINT)
	setView('files', 'viewType')
...
def changeFeq():
	...
	if not change == -1: 
		...
		wiz.LogNotify('[COLOR %s]Auto-Limpieza[/COLOR]' % COLOR1, '[COLOR %s]Frequencia %s[/COLOR]' % (COLOR2, feq[change]))

def developer():
	addFile('Convertir Textos a 0.1.7',         'converttext',           themeit=THEME1)
	addFile('Crear Codigo QR',                      'createqr',              themeit=THEME1)
	addFile('Test de Avisos',                  'testnotify',            themeit=THEME1)
	addFile('Test de Update',                         'testupdate',            themeit=THEME1)
	addFile('Test Primer Inicio',                      'testfirst',             themeit=THEME1)
	addFile('Test Ajustes del Primer Inicio',             'testfirstrun',          themeit=THEME1)
	addFile('Test Mensaje APk',             'testapk',          themeit=THEME1)
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres aplicar el guifix para:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Aplicar Fix[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]ADVERTENCIA!!!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] la compilacion no es la instalada." % (COLOR2, COLOR1, name), "Quieres aplicar el guifix igualmente?.[/COLOR]", nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Aplicar Fix[/COLOR][/B]')
		if yes:
...
def thirdPartyInstall(name, url):
	if not wiz.workingURL(url):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s] URL Invalida para la Compilacion[/COLOR]' % COLOR2); return
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Te gustaria realizar una [COLOR %s]Instalacion limpia[/COLOR] o [COLOR %s]Instalacion Normal[/COLOR] para:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR green]Instalacion limpia[/COLOR][/B]", nolabel="[B][COLOR red]Instalacion Normal[/COLOR][/B]")
	if type == 1:
	...
	if int(float(percent)) > 0:
		...
		if int(float(errors)) > 0:
			yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Completado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Te gustaria ver los errores?[/COLOR]', nolabel='[B][COLOR red]No Gracias[/COLOR][/B]',yeslabel='[B][COLOR green]Ver Errores[/COLOR][/B]')
			if yes:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox(ADDONTITLE, error)
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar Cierre[/COLOR] de kodi o [COLOR %s]Recargar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Recargar Perfil[/COLOR][/B]", nolabel="[B][COLOR red]Forzar Cierre[/COLOR][/B]")
	if todo == 1: wiz.reloadFix()
	else: wiz.killxbmc(True)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android':
		...
		display = apk
		if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
		if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]APK Installer: Url de Apk Invalida![/COLOR]' % COLOR2); return
	...
	else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: Dispositivo no Android[/COLOR]' % COLOR2)
	...
	if state in ['true', 'false']:
		...
		if not state in ['includevideo', 'includeall'] and wiz.getS('includeall') == 'true':
			try:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Tendra que desactivar [COLOR %s]Incluir TODOS los Addons[/COLOR] para desactivar[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, item))
			except:
				wiz.LogNotify("[COLOR %s]Alternar Cache[/COLOR]" % COLOR1, "[COLOR %s]Invalida id: %s[/COLOR]" % (COLOR2, state))
		else:
...
def viewLogFile():
	...
	if not oldlog == False and not mainlog == False:
		...
		if which == -1: wiz.LogNotify('[COLOR %s]Ver Log[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2); return
	elif mainlog == False and oldlog == False:
		wiz.LogNotify('[COLOR %s]Ver Log[/COLOR]' % COLOR1, '[COLOR %s]Archivo Log No Encontrado![/COLOR]' % COLOR2)
		return
...
def errorChecking(log=None, count=None, all=None):
	...
	if log == False:
		if count == None:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Archivo Log No Encontrado![/COLOR]" % COLOR2)
			return False
		else: 
			return 0
	else:
		if os.path.exists(log):
			...
			if len(match) > 0:
				...
				else: wiz.LogNotify(ADDONTITLE, "No se encontraron Errores")
			else: wiz.LogNotify(ADDONTITLE, "No se encontraron Errores")
		else: wiz.LogNotify(ADDONTITLE, "Archivo Log No Encontrado!")
...
def removeAddon(addon, name, over=False):
	if not over == False:
		...
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Estas seguro de querer borrar el addon:'% COLOR2, 'Nombre: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Borrar Addon[/COLOR][/B]', nolabel='[B][COLOR red]Canccelar[/COLOR][/B]')
	if yes == 1:
		folder = os.path.join(ADDONS, addon)
		wiz.log("Borrando Addon %s" % addon)
		wiz.cleanHouse(folder)
		xbmc.sleep(200)
		try: shutil.rmtree(folder)
		except Exception ,e: wiz.log("Error borrando %s" % addon, xbmc.LOGNOTICE)
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODA[/COLOR] la carpeta addon data almacenada en Userdata?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Eliminar addon_data[/COLOR][/B]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]'):
			wiz.cleanHouse(ADDOND)
		else: wiz.LogNotify('[COLOR %s]Eliminando Addon_Data[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2)
	elif addon == 'uninstalled':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODA[/COLOR]  la carpeta addon data almacenada en Userdata para addons desinstalados?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Eliminar addon_data[/COLOR][/B]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]'):
			total = 0
			...
			wiz.LogNotify('[COLOR %s]Addons desinstalados[/COLOR]' % COLOR1, '[COLOR %s]%s Carpetas(s) Eliminadas[/COLOR]' % (COLOR2, total))
		else: wiz.LogNotify('[COLOR %s]Borrar Addon Data[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2)
	elif addon == 'empty':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODA[/COLOR]las carpetas addon data vacias almacenadas en Userdata?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Eliminar[/COLOR][/B]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]'):
			total = wiz.emptyfolder(ADDOND)
			wiz.LogNotify('[COLOR %s]Carpetas vacias eliminadas[/COLOR]' % COLOR1, '[COLOR %s]%s Carpetas(s) borradas[/COLOR]' % (COLOR2, total))
		else: wiz.LogNotify('[COLOR %s]Carpetas vacias eliminadas[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2)
	else:
		...
		if addon in EXCLUDES:
			...
			if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar el addon_data de:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Eliminar[/COLOR][/B]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]'):
				...
				wiz.log('Addon data de %s no fue eliminado' % addon)
	wiz.refresh()

def restoreit(type):
	if type == 'build':
		x = freshStart('restore')
		if x == False: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restaurar LocalBackUp Cancelado[/COLOR]" % COLOR2); return
...
def restoreextit(type):
	if type == 'build':
		x = freshStart('restore')
		if x == False: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restaurar ExternalBackUp Cancelado[/COLOR]" % COLOR2); return
...
def buildInfo(name):
	if wiz.workingURL(BUILDFILE) == True:
		if wiz.checkBuild(name, 'url'):
			...
			msg  = "[COLOR %s]Nombre:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, name)
			msg += "[COLOR %s]Version:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, version)
			if not theme == "http://":
				themecount = wiz.themeCount(name, False)
				msg += "[COLOR %s]Tema(s):[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, ', '.join(themecount))
			msg += "[COLOR %s]Version Kodi :[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, kodi)
			msg += "[COLOR %s]Contenido Adulto:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, adult)
			msg += "[COLOR %s]Descripcion:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, description)
			wiz.TextBox(ADDONTITLE, msg)
		else: wiz.log("Nombre Invalido!")
	else: wiz.log("El archivo txt no funciona: %s" % WORKINGURL)

def buildVideo(name):
	if wiz.workingURL(BUILDFILE) == True:
		videofile = wiz.checkBuild(name, 'preview')
		if videofile and not videofile == 'http://': playVideo(videofile)
		else: wiz.log("[%s]No hay URL para la vista previa" % name)
	else: wiz.log("El archivo txt no funciona: %s" % WORKINGURL)
...
def manageSaveData(do):
	if do == 'import':
		...
		source = DIALOG.browse(1, '[COLOR %s]Seleccione la ubicacion de SaveData.zip[/COLOR]' % COLOR2, 'files', '.zip', False, False, HOME)
		if not source.endswith('.zip'):
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Error al Importar![/COLOR]" % (COLOR2))
			return
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres reemplazar el actual [COLOR %s]%s[/COLOR] file?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR green]Si Reemplazar[/COLOR][/B]", nolabel="[B][COLOR red]No Saltar[/COLOR][/B]"): continue
					else: os.remove(old)
		...
		os.remove(tempfile)
		if x == 0: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Fallo al importar datos[/COLOR]" % COLOR2)
		else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Importacion de datos Completa[/COLOR]" % COLOR2)
	elif do == 'export':
		...
		source = DIALOG.browse(3, '[COLOR %s]Seleccione donde desea exportar el archivo zip?[/COLOR]' % COLOR2, 'files', '', False, True, HOME)
		...
		zipf.close()
		if source == mybuilds:
			DIALOG.ok(ADDONTITLE, "[COLOR %s]El BackUp de Save data se ha guardado en:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
		else:
			try:
				xbmcvfs.copy(tempzip, os.path.join(source, 'SaveData.zip'))
				DIALOG.ok(ADDONTITLE, "[COLOR %s]El BackUp de Save data se ha guardado en:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, os.path.join(source, 'SaveData.zip')))
			except:
				DIALOG.ok(ADDONTITLE, "[COLOR %s]El BackUp de Save data se ha guardado en:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restablecer la" % COLOR2, "Configuracion de Kodi a los valores por defecto", "Antes de instalar el backup?[/COLOR]", nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Continuar[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restablecer la" % COLOR2, "Configuracion de Kodi a los valores por defecto", "Antes de instalar [COLOR %s]%s[/COLOR]?" % (COLOR1, install), nolabel='[B][COLOR red]No, Cancelara[/COLOR][/B]', yeslabel='[B][COLOR green]Continuar[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restablecer la" % COLOR2, "Configuracion de Kodi a los valores por defecto?[/COLOR]", nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Continuar[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
		...
		DP.create(ADDONTITLE,"[COLOR %s]Calculando archivos y carpetas" % COLOR2,'', 'Espere![/COLOR]')
		total_files = sum([len(files) for r, d, files in os.walk(xbmcPath)]); del_file = 0
		DP.update(0, "[COLOR %s]Lista de exclusiones." % COLOR2)
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			dirs[:] = [d for d in dirs if d not in EXCLUDES]
			for name in dirs:
				DP.update(100, '', 'Borrando carpetas vacias: [COLOR %s]%s[/COLOR]' % (COLOR1, name), '')
		...
		if over == True:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar cierre[/COLOR] de kodi o [COLOR %s]Recargar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar Perfil[/COLOR][/B]", nolabel="[B][COLOR green]Forzar cierre[/COLOR][/B]")
...
####THANKS GUYS @ NaN #######
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Borrar Cache[/COLOR][/B]'):
		wiz.clearCache()
		#clearThumb()

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar cache, packages y thumbnails?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelaar[/COLOR][/B]',yeslabel='[B][COLOR green]Borrar todo[/COLOR][/B]'):
		wiz.clearCache()
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar %s y la carpeta de Thumbnails?' % (COLOR2, latest), "Se regeneraran al siguiente inicio[/COLOR]", nolabel='[B][COLOR red]No Borrar[/COLOR][/B]', yeslabel='[B][COLOR green]Borrar Thumbs[/COLOR][/B]')
	if choice == 1:
...
def testnotify():
	...
	if url == True:
		try:
			...
			if id == False: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Avisos: no formateado correctamente[/COLOR]" % COLOR2); return
			notify.notification(msg, True)
		except Exception, e:
			wiz.log("Error en la ventana de Avisos: %s" % str(e), xbmc.LOGERROR)
	else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]URL para Avisos Invalida[/COLOR]" % COLOR2)
...
def speedMenu():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.program.StarWZRD/speedtest.py")')
